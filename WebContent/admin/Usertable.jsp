...
<script type="text/javascript">
	function xoa(ten,idsp)
	{
		...
		var x=confirm("Bạn có muốn xóa Sản phẩm: "+ten);
		  if(x)
			{
			 window.location.href="${pageContext.request.contextPath}/quanli?action=3&idsp="+idsp;
			 }
	}
	function them()
	{
		window.location.href="${pageContext.request.contextPath}/quanli?action=2";
	}
</script>
    ...
    <div id="main-wrapper">
      ...
      <div class="col-sm-12 col-md-7">
        <div class="dataTables_paginate paging_simple_numbers" id="zero_config_paginate">
            <ul class="pagination">
                <li class="paginate_button page-item previous " id="zero_config_previous"><a href="?page=${so-1}&action=4" class="page-link">Previous</a></li>
               
           		<c:forEach var="i" begin="1" end="${trang}">
           		
                        <li class="paginate_button page-item  <c:if test="${so==i}"><c:out value="${' active'}"></c:out></c:if>"><a href="?page=${i}&action=4" class="page-link"><c:out value="${i}"></c:out></a></li>
                </c:forEach>
                
                <li class="paginate_button page-item next" id="zero_config_next"><a href="?page=${so+1}&action=4"  class="page-link">Next</a></li>
            </ul>
        </div>
    </div>
                                        ...
                                        <tbody>
                                            ...
                                            <c:forEach items="${list}" var="i">
	                                            <tr>
		                                                ...
		                                                <td><img src="<c:out value="${i.getAvatar().trim()}"></c:out>" style="width: 120px;height: auto;"></td>
		                                                 <td> <c:out value="${i.getId_user().trim()}"></c:out> </td>
		                                                <td><div style="width: 150px;"><c:out value="${i.getTenhienthi().trim()}"></c:out></div></td>
		                                                <td><c:out value="${i.getSdt().trim()}"></c:out></td>
		                                                <td><c:out value="${i.getNgaysinh()}"></c:out></td>
